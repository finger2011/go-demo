...
//RequestResponse request resoponse
...
func startServer(wg *sync.WaitGroup) {
	tcpAddr, err := net.ResolveTCPAddr("tcp", "127.0.0.1:19999")
	if err != nil {
		fmt.Printf("create tcp addr error:%v", err)
		return
	}
	tcpListener, err := net.ListenTCP("tcp", tcpAddr)
	if err != nil {
		fmt.Printf("create tcp listener error:%v", err)
		return
	}
...
func startClient(wg *sync.WaitGroup) *net.TCPConn {
	...
	tcpAddr, err := net.ResolveTCPAddr("tcp", "127.0.0.1:19999")
	if err != nil {
		fmt.Printf("create client tcp addr error:%v", err)
		return nil
	}
	conn, err := net.DialTCP("tcp", nil, tcpAddr)
	if err != nil {
		fmt.Printf("create client tcp dial error:%v", err)
		return nil
	}
